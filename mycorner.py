...
__all__ = ["corner", "hist2d", "quantile", "mostcompact_68"]
...
def corner(xs, bins=20, range=None, weights=None, color="k",
           ...
           scale_hist=False, quantiles=None, quantiles_68=False, verbose=False, fig=None,
    ...
    quantiles_68 : bool
        If true, calcualte the errors using the 68 most compact method.
    ...
    **hist2d_kwargs
    ...
    trdim = 0.8 * factor   # size of top/right margin
    ...
    for i, x in enumerate(xs):
        ...
        # Plot the histograms.
        if smooth1d is None:
            n,nbins = np.histogram(x, bins=bins[i], weights=weights,
                              range=np.sort(range[i]))
            n = n/float(n.max())
            #print (n.max())
            ax.step(nbins[0:-1],n, where='post', color=color)
...
def mostcompact_68(x):
    """Compute sample euqal probability errors.
    ...
    Returns
    -------
    qvalues :  list [low_lim, peak_distrib, upper_lim]

    """

    x = np.array(np.sort(x, axis=0))
    lenx = len(x)
    seper = int(0.68*lenx)
    nbins = 45#int(lenx**(1/3.))
    qvalues = []
    #for i in range (x.shape[1]):
    up_old = x[seper]
    low_old = x[0]
    range_old = x[seper] -x[0]
    for ii in range(int(0.32*lenx)):
        q_up = x[seper+ii]
        q_low = x[ii]
        ran =  q_up -q_low
        if ran < range_old:
    ...
    index = np.where(x == low_old)[0][0]
    y, bins = np.histogram(x[index:seper+index], bins=nbins,  density=True)
    #y, bins = np.histogram(x, bins=nbins,  density=True)
    bins_middle = bins[0:-1] + abs(bins[0:-1]-bins[1::])/2
    #print (bins_middle)
    q_max = bins_middle[y.argmax()]
    qvalues.append(low_old)
    qvalues.append(q_max)
    qvalues.append(up_old)
    #print ('qvals', qvalues)
...
def hist2d(x, y, bins=20, range=None, weights=None, levels=None, smooth=None,
    ...
    if plot_datapoints:
        ...
        ax.plot(x, y, "o", zorder=-1, rasterized=True3, **data_kwargs)
