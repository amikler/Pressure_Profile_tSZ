...
def patches_ana (img, rmax, cropvalue=np.inf, num=1000):
	idx = 0
	patchescoll = np.zeros(shape=(num, 2*rmax+1, 2*rmax+1))
	while idx < num:
		xcent, ycent = np.random.randint(0+rmax,img.shape[0]-rmax, 2)
		patch_random = img[ycent-rmax:ycent+rmax+1,xcent-rmax:xcent+rmax+1]
		if np.any(patch_random == cropvalue):
			xcent, ycent = np.random.randint(0,img.shape[0],  2)
			patch_random = img[ycent-rmax:ycent+rmax+1,xcent-rmax:xcent+rmax+1]
		if np.all(patch_random != cropvalue) and patch_random.shape == patchescoll[0,:,:].shape:
			patchescoll[idx,:,:] = patch_random
			idx = idx+1
